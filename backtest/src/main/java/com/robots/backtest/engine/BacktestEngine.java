...
public class BacktestEngine {
    ...
    public Results launch(OhlcTimeserie ohlc, Timeserie signalLong) {
        ...
        results.addEquity(new Point(indexes.get(0), 0));
        ...
        for (int i = 1; i<indexes.size()-1; i++) {
            ...
            if (isOngoingTrade) {
                if (direction == TradeType.LONG) {
                    ...
                    results.addEquity(new Point(index, previousValue + pnl));

                    // close long trade if needed
                    if (signalLongMap.get(index).getValue() == 0) {
